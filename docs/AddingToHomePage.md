...
and [better-npm-run](https://www.npmjs.com/package/better-npm-run), runs 
 `./webpack/webpack.config.js` on port 3001; runs `./bin/server.js` for HTTP on port 3000; 
...
* `./webpack/webpack.config` does teh actual compilation with the 
...
[piping](https://www.npmjs.com/package/piping) module, triggering the webpack.config to rebuild 
